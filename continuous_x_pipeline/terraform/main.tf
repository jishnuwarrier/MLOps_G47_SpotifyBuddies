resource "openstack_networking_network_v2" "private_net" {
  name                  = "private-net-devops-${var.suffix}"
  port_security_enabled = false
}

resource "openstack_networking_subnet_v2" "private_subnet" {
  name       = "private-subnet-devops-${var.suffix}"
...
resource "openstack_networking_port_v2" "private_net_ports" {
  for_each              = var.nodes
  name                  = "port-${each.key}-devops-${var.suffix}"
  network_id            = openstack_networking_network_v2.private_net.id
  port_security_enabled = false
...
resource "openstack_networking_port_v2" "sharednet2_ports" {
  for_each   = var.nodes
    name       = "sharednet2-${each.key}-devops-${var.suffix}"
...
resource "openstack_compute_instance_v2" "nodes" {
  ...
  user_data = <<-EOF
    #! /bin/bash
    sudo echo "127.0.1.1 ${each.key}-devops-${var.suffix}" >> /etc/hosts
...
resource "openstack_networking_floatingip_v2" "floating_ip" {
  pool        = "public"
  description = "devops IP for ${var.suffix}"
