...
# Define the test range of user IDs
test_user_ids = list(range(1, 10001))  # IDs from 1 to 1000
...
avg_latency = total_time / num_users
...
print(f"Average latency per user: {avg_latency:.6f} seconds")
print(f"Throughput: {throughput:.2f} users/second")

# Optionally inspect a few results
print("\n--- Sample Inference Results ---")
for user_id in test_user_ids[:5]:  # show top 5 results
    playlists = ranked_results.get(user_id, [])
    print(f"User {user_id} → Top-{len(playlists)} Playlists: {playlists}")
